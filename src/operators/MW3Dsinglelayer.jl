...
struct FMMMatrixMWSL{K,NearMatrixType} <: LinearMaps.LinearMap{K}
    fmm::ExaFMMt.ExaFMM{K}
    fmm_t::ExaFMMt.ExaFMM{K}
    B1::SparseMatrixCSC{real(K),Int}
    B2::SparseMatrixCSC{real(K),Int}
    B3::SparseMatrixCSC{real(K),Int}
    Bdiv::SparseMatrixCSC{real(K),Int}
    B1_test::SparseMatrixCSC{real(K),Int}
    B2_test::SparseMatrixCSC{real(K),Int}
    B3_test::SparseMatrixCSC{real(K),Int}
    Bdiv_test::SparseMatrixCSC{real(K),Int}
    correctednears::NearMatrixType
    dim::Tuple{Int,Int}

    function FMMMatrixMWSL{K}(
        fmm,
        fmm_t,
        B1,
        B2,
        ...
        rowdim,
        coldim,
    ) where {K,eltype(correctednears),typeof(correctednears)}
        return new{K}(
            fmm,
            fmm_t,
            B1,
