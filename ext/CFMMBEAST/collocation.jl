...
"""
    potentials(qp::Matrix, X::BEAST.RTBasis)
...
"""
    curlpotentials(qp::Matrix, X::BEAST.Space)
...
"""
function curlpotentials(qp::Matrix, X::BEAST.Space)
    rfspace = refspace(X)
    _, tad, _ = assemblydata(X)
    len = length(qp) * length(qp[1, 1]) * size(tad.data)[1] * size(tad.data)[2]
    rc = ones(Int, len, 2)
...
"""
    divpotentials(qp::Matrix, X::BEAST.Space)
...
# Arguments
  ...
  - `X::BEAST.Space`: Basis function.
"""
function divpotentials(qp::Matrix, X::BEAST.Space)
    rfspace = refspace(X)
    _, tad, _ = assemblydata(X)
    len = length(qp) * length(qp[1, 1]) * size(tad.data)[1] * size(tad.data)[2]
    rc = ones(Int, len, 2)
