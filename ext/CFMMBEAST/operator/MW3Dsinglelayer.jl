...
function (::CorrectionFactorMatrixMethod.ExaFMMtFunctor)(
    ...
    return CorrectionFactorMatrixMethod.CFMMMatrixMW3DSingleLayer{scalartype(operator)}(
...
function MW3DSLpotentialmatrix(
    ...
    divBtest = dropzeros(sparse(rcdiv[:, 2], rcdiv[:, 1], valsdiv))
    ...
    divBtrial = dropzeros(sparse(rcdiv[:, 1], rcdiv[:, 2], valsdiv))
